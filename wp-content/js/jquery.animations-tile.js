...
(function($, window, document, undefined) {
    function tile(wrapper, element, rows, cols) {
        ...
        for (var i = 0; i < rows; ++i) {
            ...
            for (var j = 0; j < cols; ++j) {
                ...
                else { var clone = i == 0 && j == 0 ? element : element.clone(); var container = $.wrap(clone);
                    tile = $.wrap(container); }
                rowTiles.push(tile);
                wrapper.append(tile);
            }
    ...
    function styleTiles(element, rows, cols, tiles) {
        ...
        for (var i = 0; i < rows; ++i) {
            ...
            for (var j = 0; j < cols; ++j) {
                ...
                var x = -offsetX;
                offsetX += useWidth;
                if (isImg) { tile.css({ width: useWidth, height: useHeight, display: 'inline-block', "float": 'left', 'background-position': x + 'px ' + y + 'px', 'background-size': width + 'px ' + height + 'px' }); } else { var container = tile.children().first();
                    container.css({ width: width, height: height, display: 'inline-block' });
                    container.vendorCss('transform', 'translate(' + x + 'px, ' + y + 'px)');
                    tile.css({ width: useWidth, height: useHeight, overflow: 'hidden', display: 'inline-block', "float": 'left' }); }
            }
        }
    }
    ...
    function shuffle(array) {
        for (var i = 0; i < array.length; ++i) { var r = parseInt(Math.random() * array.length); var tmp = array[i];
            array[i] = array[r];
            array[r] = tmp; }
        return array;
    }
    ...
    var sequenceMethods = {
        ...
        rl: function(rows, cols) { return sequenceMethods.lr(rows, cols).reverse(); },
        ...
        bt: function(rows, cols) { return sequenceMethods.tb(rows, cols).reverse(); },
        ...
        rlbt: function(rows, cols) { return sequenceMethods.lrtb(rows, cols).reverse(); },
        ...
        lrbt: function(rows, cols) { return sequenceMethods.rltb(rows, cols).reverse(); },
        ...
        btrl: function(rows, cols) { return sequenceMethods.tblr(rows, cols).reverse(); },
        ...
        btlr: function(rows, cols) { return sequenceMethods.tbrl(rows, cols).reverse(); },
        random: function(rows, cols) { return shuffle(sequenceMethods.lrtb(rows, cols)) },
        randomCols: function(rows, cols) { return shuffle(sequenceMethods.lr(rows, cols)) },
        randomRows: function(rows, cols) { return shuffle(sequenceMethods.tb(rows, cols)) }
    };
    var animation = {
        duration: 2000,
        timeout: 1000,
        emptyAnimation: true,
        wrap: true,
        variables: { rows: 1, cols: 1, effect: 'flyOut', sequent: true, sequence: 'random', groups: 1, adjustDuration: true },
        prepare: function(options) {
            ...
            var fail = function() { element.stop(); };
            if ($.isFunction(effects)) {
                for (var i = 0; i < sequences.length; ++i) {
                    var step = sequences[i];
                    for (var j = 0; j < step.length; ++j) { var pair = step[j]; var row = pair[0]; var col = pair[1]; var cloneOptions = $.extend({ fail: fail }, subOptions); var effect = effects.call(this, cloneOptions, row, col);
